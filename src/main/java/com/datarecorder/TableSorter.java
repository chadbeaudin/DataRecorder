...
public class TableSorter extends AbstractTableModel {
    ...
    private static final long serialVersionUID = -9191187640786824484L;
    ...
    public TableSorter(final DefaultTableModel tableModel) {
        this();
        setTableModel(tableModel);
    }

    public TableSorter(final DefaultTableModel tableModel, final JTableHeader tableHeader) {
        this();
    ...
    /**
     * Retrieves the new datavector. The data is rearranged to return the true model that the user is seeing since the
     * underlying model order never changes with a sort.
     * 
     * @return the table vector (vector of vectors)
     */
    public Vector<Object> getDataVector() {
        final Vector<Object> tempVector = new Vector<Object>();
        if (getViewToModel() != null) {
            for (int i = 0; i < getViewToModel().length; i++) {
                tempVector.add(tableModel.getDataVector().elementAt(getViewToModel()[i].modelIndex));
            }
        }
        return tempVector;
    }
    ...
    public void removeRow(final int i) {
        tableModel.removeRow(getViewToModel()[i].modelIndex);
    }

    public void addRow(final Vector<Object> v) {
        tableModel.addRow(v);
    }
    ...
    public void setColumnComparator(final Class<Object> type, final Comparator<Object> comparator) {
        if (comparator == null) {
    ...
    protected Comparator<Object> getComparator(final int column) {
        final Class<Object> columnType = (Class<Object>) tableModel.getColumnClass(column);
    ...
    @Override
    public Class<Object> getColumnClass(final int column) {
        return (Class<Object>) tableModel.getColumnClass(column);
    }
    ...
    private static class Arrow implements Icon {
        ...
        public void paintIcon(final Component c, final Graphics g, final int x, int y) {
            ...
            final int dx = size / 2 * (int) Math.pow(0.8, priority);
